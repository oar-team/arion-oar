...
common = {
  ...
  nixos.configuration = {
    ...
    [ traefik jq curl telnet pkgs.nur.repos.kapack.oar (python37.withPackages(ps: with ps;
      [ pip psycopg2 clustershell pyzmq click pyinotify sortedcontainers jupyterlab pkgs.nur.repos.kapack.oar])) ];
...

{
  ...
  services.server = addCommon {
    ...
    nixos.configuration = {
      ...
      services.oar.web.proxy.entryPointHttp = "server:5000";
    };
    
    #service.capabilities = { SYS_ADMIN = true; NET_BIND_SERVICE = true; }; # for phpfpm need to drawgantt
    service.capabilities = { SYS_ADMIN = true; NET_BIND_SERVICE = true; SYS_RESOURCE = true; }; # for phpfpm need to drawgantt
